...
function ScoreRing({ score }) {
  ...
  const color = score >= 75 ? "#22c55e" : score >= 50 ? "#FF8C00" : "#ef4444";
  useEffect(() => { const t = setTimeout(() => setAnimated(true), 100); return () => clearTimeout(t); }, []);
  return (
    <div style={{ position: "relative", width: 80, height: 80, flexShrink: 0 }}>
      <svg width="80" height="80" style={{ transform: "rotate(-90deg)" }}>
        <circle cx="40" cy="40" r={r} fill="none" stroke="rgba(10,35,66,0.1)" strokeWidth="6" />
        <circle cx="40" cy="40" r={r} fill="none" stroke={color} strokeWidth="6"
          strokeDasharray={`${fill} ${circ}`}
          style={{ transition: "stroke-dasharray 1.2s cubic-bezier(0.4,0,0.2,1)" }} />
      </svg>
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontSize: 20, fontWeight: 800, color, fontFamily: "'Barlow Condensed', sans-serif" }}>{score}</span>
...
export default function AnalysisCard({ analysis }) {
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid rgba(10,35,66,0.1)", borderRadius: "var(--radius-lg)", padding: "24px 28px", marginBottom: 24, animation: "fadeUp 0.4s ease forwards", boxShadow: "0 2px 12px rgba(10,35,66,0.06)" }}>
      <div style={{ display: "flex", gap: 24, alignItems: "flex-start", flexWrap: "wrap" }}>
        <ScoreRing score={analysis.score} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 16, color: "var(--navy)", marginBottom: 14, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", textTransform: "uppercase", letterSpacing: 0.5 }}>
